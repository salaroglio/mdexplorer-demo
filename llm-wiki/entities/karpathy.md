---
title: Andrej Karpathy
kind: entity
tags: [persone, ricerca-ai, openai, tesla]
last_updated: 2026-05-10
---

# Andrej Karpathy

## Sintesi

Andrej Karpathy è un ricercatore di intelligenza artificiale, co-fondatore di OpenAI e già Director of AI presso Tesla. Ad Aprile 2026 ha pubblicato un gist su GitHub che descrive il pattern **LLM Wiki**, un approccio al knowledge management dove un agente AI mantiene attivamente un wiki strutturato in markdown invece di fare retrieval su documenti grezzi ad ogni query.

## Punti chiave

- È stato co-fondatore di OpenAI [fonte: [sources/2026-04-karpathy-gist](../sources/2026-04-karpathy-gist.md)]
- È stato Director of AI in Tesla, dove ha guidato il team Autopilot [fonte: pubblico, profilo Twitter/X]
- Ad Aprile 2026 ha proposto il pattern **LLM Wiki** in un gist pubblico [fonte: [sources/2026-04-karpathy-gist](../sources/2026-04-karpathy-gist.md)]
- Il suo wiki personale ha raggiunto 100 articoli e 400.000 parole [fonte: [sources/2026-04-beyond-rag-article](../sources/2026-04-beyond-rag-article.md)]
- Frase distintiva (adattata al substrato che si usa): *"\[Editor markdown\] è l'IDE; l'LLM è il programmatore; il wiki è il codebase"* — con MdExplorer diventa: *"MdExplorer è l'IDE; l'LLM è il programmatore; il wiki è il codebase"* [fonte: [sources/2026-04-karpathy-gist](../sources/2026-04-karpathy-gist.md)]

## Vedi anche

- [LLM Wiki](../concepts/llm-wiki.md) — il pattern che ha proposto
- [Knowledge Compounding](../concepts/knowledge-compounding.md) — l'idea fondamentale
- [RAG vs Wiki](../concepts/rag-vs-wiki.md) — il confronto con l'approccio tradizionale

## Storico

- 2026-05-10 — pagina creata da ingest di `sources/2026-04-karpathy-gist`
